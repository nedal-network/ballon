...
<div class="flex flex-col h-[calc(100vh-64px)] gap-y-8 py-8">
        ...
        <div class="grid overflow-auto custom-table rounded-xl bg-white shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10"
            ...
            @foreach ($record->coupons as $coupon)
                ...
                <label for="coupon-{{ $coupon->id }}" class="tbody min-w-1 @if($isCheckedAlready || $coupon->missingData) bg-zinc-100 text-zinc-400 dark:bg-white/10" @else " style="background: {{ $backgroundColor }}; color: {{ $textColor }}" @endif><button style="width: 35px; --c-400:var(--primary-400);--c-500:var(--primary-500);--c-600:var(--primary-600);" class="bg-custom-600 dark:bg-custom-500 dark:focus-visible:ring-custom-400/50 dark:hover:bg-custom-400 duration-75 fi-ac-action fi-ac-btn-action fi-btn fi-btn-color-primary fi-btn-size-md fi-color-custom fi-color-primary fi-size-md focus-visible:ring-2 focus-visible:ring-custom-500/50 font-semibold gap-1.5 grid-flow-col hover:bg-custom-500 inline-grid items-center justify-center outline-none relative rounded shadow-sm text-sm text-white transition py-1 mb-2" @click="navigator.clipboard.writeText('{{ implode(';',$coupon->passengers->where('email','!=','')->pluck('email')->toArray()) }}'); new FilamentNotification().title('Email címek vágólapra másolva').icon('tabler-mail-forward').success().send()"><x-tabler-mail-forward style="height: 16px;" /></button><br><button style="width: 35px; --c-400:var(--primary-400);--c-500:var(--primary-500);--c-600:var(--primary-600);" class="bg-custom-600 dark:bg-custom-500 dark:focus-visible:ring-custom-400/50 dark:hover:bg-custom-400 duration-75 fi-ac-action fi-ac-btn-action fi-btn fi-btn-color-primary fi-btn-size-md fi-color-custom fi-color-primary fi-size-md focus-visible:ring-2 focus-visible:ring-custom-500/50 font-semibold gap-1.5 grid-flow-col hover:bg-custom-500 inline-grid items-center justify-center outline-none relative rounded shadow-sm text-sm text-white transition py-1 mt-2" @click="navigator.clipboard.writeText('{{ implode(';',$coupon->passengers->where('phone','!=','')->pluck('phone')->toArray()) }}'); new FilamentNotification().title('Telefonszámok vágólapra másolva').icon('tabler-device-mobile').success().send()"><x-tabler-device-mobile style="height: 16px;" /></button></label>
